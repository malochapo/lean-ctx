...
mod tests {
    ...
    // state another test may have left behind. (Sibling temp dirs flake here:
    // under `--test-threads=1` a prior test can allow-list the temp directory
    // via `LEAN_CTX_*` env vars, so the out-of-jail sibling is accepted.)
    //
